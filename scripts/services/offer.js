...
app.factory('Offer', function($firebase, FURL, Auth, $q, Task) {
  ...
  var Offer = {
    ...
    acceptOffer: function(taskId, offerId, runnerId) {
      ...
      return o.$update({accepted: true}).then(function(){
        ...
        t.$update({status:'assigned', runner: runnerId}).then(function(){
    ...
    notifyRunner: function(taskId,runnerId){
      Auth.getProfileInfo(runnerId).$loaded().then(function(runner){
        var ru = {
          email : runner.email,
          taskId : taskId,
          name : runner.name
